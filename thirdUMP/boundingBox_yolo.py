...
def parse_detections(metadata: dict):
    ...
    # The boxes are likely normalized to [0, 1] or in input tensor coordinates
    img_width = metadata.get('ScalerCrop')[2]  # Width
    img_height = metadata.get('ScalerCrop')[3]  # Height
    ...
    for i in range(len(scores)):
        if scores[i] > threshold:
            box = boxes[i]
            y1, x1, y2, x2 = box  # Note: Often boxes are in [y1, x1, y2, x2] format
            
            # Scale coordinates to image dimensions (if normalized)
            x1 = int(x1 * img_width)
